...
GET /api/batches/<batch_id>/recordings/<recording_id>/details/
"""
...
@bp.route(
    "/api/batches/<batch_id>/recordings/<path:recording_id>/details/",
    methods=["GET"],
)
@bp.route(
    "/api/batches/<batch_id>/recordings/<path:recording_id>/details",
...
@bp.route(
    "/api/medsum-test/batches/<batch_id>/recordings/<path:recording_id>/details/",
    methods=["GET"],
)
@bp.route(
    "/api/medsum-test/batches/<batch_id>/recordings/<path:recording_id>/details",
...
def get_recording_details(batch_id: str, recording_id: str):
    """Fact-level missed/wrong/invented breakdown for one recording."""
    test_type, model, batch_ids = _query_filters()
    calc = AccuracyCalculator(
        batch_id,
        test_type=test_type,
    ...
    if not calc.batch_exists():
        return _missing_batch(batch_id)
    payload = calc.get_recording_details(recording_id)
    if payload is None:
        return jsonify({
            "status": "error",
